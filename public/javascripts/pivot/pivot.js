(function() {
  ...
  callWithJQuery(function($) {
    ...
    aggregators = (function(tpl) {
      return {
        "总数": tpl.count(usFmtInt),
        "单值总数": tpl.countUnique(usFmtInt),
        ...
        "总和百分比": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
        "行总和百分比": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
        "列总和百分比": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
        "总数百分比": tpl.fractionOf(tpl.count(), "total", usFmtPct),
        "行总数百分比": tpl.fractionOf(tpl.count(), "row", usFmtPct),
        "列总数百分比": tpl.fractionOf(tpl.count(), "col", usFmtPct)
      };
    })(aggregatorTemplates);
    renderers = {
    ...
    renderers = $.extend(renderers,$.echart_renderers);  //集成echarts渲染器
    locales = {
      en: {
    ...
    derivers = {
      ...
      dateFormat: function(col, formatString, utcOutput, mthNames, dayNames) {
        ...
        return function(record) {
          var date;
          if(record[col] && record[col].length==13){
        	  //返回时间是13为long类型
        	  date = new Date(new Number(record[col]));
          }else{
     ...
     */
    $.fn.pivot = function(input, opts, ecOpts) {
      // if(input && input[0] && input[0] instanceof Array){
      //   input = PivotData.convertToArray(input);
      // }
      // if(opts.rendererName && !opts.renderer){
      //   opts.renderer = locales['en'].renderers[opts.rendererName];
      //   opts.aggregator = locales['en'].aggregators[opts.aggregatorName];
      // }

      var defaults, e, error, error1, pivotData, result, x, locale;
      defaults = {
        cols: [],
        rows: [],
        vals: [],
      ...
      try {
        ...
        try {
          result = opts.renderer(pivotData, opts.rendererOptions, ecOpts);
        } catch (error) {
          e = error;
      ...
      if(echarts && result && result.baseOption && result.baseOption.series){
        //获取页面可用宽高
        var height = $(window).height();
        var width = $(window).width();
        if(width/height > 1)
          $(this).css({'width': '100%', 'height': height/2});
        else
          $(this).css({'width': '100%', 'height': height/3.5});
        return echarts                                                  //如果返回的是echart参数,用echart渲染
            .init(this[0])
            .setOption(result);
      }
     ...
     */
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale ,ecOpts, noShowUI) {
      var a, aggregator, attrLength, axisValues, c, colList, defaults, e, error, existingOpts, fn, i, initialRender, k, l, len1, len2, len3, len4, n, o, opts, pivotTable, q, ref, ref1, ref2, ref3, ref4, refresh, refreshDelayed, renderer, rendererControl, shownAttributes, tblCols, tr1, tr2, uiTable, unusedAttrsVerticalAutoCutoff, unusedAttrsVerticalAutoOverride, x, extendOpts;
      if (overwrite == null) {
        overwrite = false;
      }
      if (locale == null) {
      ...
      if ((existingOpts == null) || overwrite) {
        ...
        opts = $.extend(existingOpts, extendOpts);   //原始代码：opts = existingOpts;
      }
      try {
        input = PivotData.convertToArray(input);
        tblCols = (function() {
        ...
        var title = $("<div>备选字段:</div>")
        colList = $("<td>").addClass('pvtAxisContainer pvtUnused').append(title);
        ...
        $("<td>").addClass('pvtAxisContainer pvtHorizList pvtCols').append($("<div>列字段:</div>")).appendTo(tr1);
        tr2 = $("<tr>").appendTo(uiTable);
        
        tr2.append($("<td>").addClass('pvtAxisContainer pvtRows').attr("valign", "top").append($("<div>行字段:</div>")));
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            if(noShowUI)
              _this.pivot(input, subopts, ecOpts);
            else
              pivotTable.pivot(input, subopts, ecOpts);

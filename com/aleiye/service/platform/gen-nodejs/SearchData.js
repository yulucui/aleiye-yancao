...
SearchData_query2CSV_args = function(args) {
  ...
  if (args) {
    ...
    if (args.csvParameter !== undefined) {
      this.csvParameter = args.csvParameter;
    }
  }
};
...
SearchData_scanData_args = function(args) {
  ...
  if (args) {
    if (args.queryModel !== undefined) {
      this.queryModel = args.queryModel;
    }
    if (args.scanModel !== undefined) {
      this.scanModel = args.scanModel;
    }
  }
};
...
SearchData_queryByUpdate_args = function(args) {
  ...
  if (args) {
    ...
    if (args.updateModel !== undefined) {
      this.updateModel = args.updateModel;
    }
  }
};
...
SearchDataClient.prototype.getIndexNames = function(from, to, userId, database, isProcessTime, callback) {
  this._seqid = this.new_seqid();
  if (callback === undefined) {
    var _defer = Q.defer();
    this._reqs[this.seqid()] = function(error, result) {
    ...
    this.send_getIndexNames(from, to, userId, database, isProcessTime);
    return _defer.promise;
  } else {
    this._reqs[this.seqid()] = callback;
    this.send_getIndexNames(from, to, userId, database, isProcessTime);
...
SearchDataClient.prototype.send_getIndexNames = function(from, to, userId, database, isProcessTime) {
  var output = new this.pClass(this.output);
...
SearchDataProcessor.prototype.process_getIndexNames = function(seqid, input, output) {
  ...
  if (this._handler.getIndexNames.length === 5) {
    Q.fcall(this._handler.getIndexNames, args.from, args.to, args.userId, args.database, args.isProcessTime)
      .then(function(result) {
    ...
    this._handler.getIndexNames(args.from, args.to, args.userId, args.database, args.isProcessTime,  function (err, result) {
